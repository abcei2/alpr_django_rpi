...
lib = CDLL("/home/santi/ECUADOR/alpr-unconstrained/darknet/libdarknet.so", RTLD_GLOBAL)
...
def detect_plates(frame, net, meta, wpod_net, lp_threshold, letter_threshold):
    ...
    side = int(ratio*288.-60)
    bound_dim = min(side + (side % (2**4)), 608.-60)
    ...
    for i in range(len(LlpImgs)):
        ...
        for det in r[0]:
            ...
            cv2.imwrite('static/plate_proccesed.jpg', Ilp)
        while 0 in posicion:
            posicion.remove(0)
